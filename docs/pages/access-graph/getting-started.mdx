...
* A Teleport cluster with Auth and Proxy services upgraded to master (TODO: v14.3.X+ or v15+)
...
* Access Graph service executable / Docker image, which can be retrieved from TODO
...
Access Graph is configured automatically for Teleport Team users.
Skip directly to [step 3](#step-33-view-the-access-graph-in-web-ui)
to get started with Access Graph in the Web UI.

</TabItem>
</Tabs>

## Step 1/3. Setting up Teleport Access Graph service
...
```console
$ curl 'https://<Var name="teleport.example.com" />/webapi/auth/export?type=tls-host' > /etc/teleport_host_ca.pem
```
</TabItem>
...
$ tctl get cert_authorities --format=json \
    ...
    | base64 -d > /etc/teleport_host_ca.pem
```
</TabItem>
</Tabs>
...
backend:
  postgres:
    ...
    # e.g. `sslmode=verify-full&sslrootcert=/etc/my_postgres_ca.crt`.
...
# This is the default value, and this key can be omitted to listen on port 50051 on all interfaces.
address: ":50051" 

tls:
  # File paths of PEM-encoded TLS certificate and private key for the TAG server
  cert: /etc/access_graph.crt
  key: /etc/access_graph.key
...
registration_cas:
  - /etc/teleport_host_ca.pem # A full path to the file containing the Teleport cluster's host CA certificate.
```

Finally, start the TAG service as follows:

TODO: docker instructions here 
...
## Step 2/3. Updating Teleport Auth server configuration
...
## Step 3/3. View the Access Graph in Web UI
